...
class VanillaPGAgent(base_agent.BaseAgent):
    NAME = "VanillaPG"
    ...
    def _calc_return(self, r: torch.Tensor, done: torch.Tensor) -> torch.Tensor:
        """
        Calculates the basic discounted return values for each episode.
        ...
        Returns:
            torch.Tensor: A tensor containing the calculated return values for each timestep within its respective episode.
        """
        ...
        for end_idx in episode_ends:
            total_return = sum(self._discount ** (k - start_idx) * r[k] for k in range(start_idx, end_idx + 1))

            # Assign total return value to each timestep within the respective episode
            return_t[start_idx : end_idx + 1] = total_return
